...
// seÃ±al a usar para las notificaciones de resoluciÃ³n
...
selector_init(const struct selector_init  *c) {
    ...
    // principal. La tÃ©cnica se encuentra descripta en
    ...
    // 1. Registramos una funciÃ³n que atenderÃ¡ la seÃ±al de interrupciÃ³n
    //    del selector.
    //    Esta interrupciÃ³n es Ãºtil en entornos multi-hilos.
...
/* tarea bloqueante */
struct blocking_job {
    /** selector dueÃ±o de la resolucion */
    fd_selector  s;
    /** file descriptor dueÃ±o de la resolucion */
    int fd;
...
/** marca para usar en item->fd para saber que no estÃ¡ en uso */
static const int FD_UNUSED = -1;

/** verifica si el item estÃ¡ usado */
...
struct fdselector {
    ...
    // esto podrÃ­a mejorarse utilizando otra estructura de datos
    struct item    *fds;
    ...
    // notificaciÃ³nes entre blocking jobs y el selector
...
selector_select(fd_selector s) {
    ...
    if(-1 == fds) {
        switch(errno) {
            case EAGAIN:
            case EINTR:
                // si una seÃ±al nos interrumpio. ok!
                break;
            case EBADF:
